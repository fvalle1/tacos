...
{
    cout << "Generating" << maxStatistic << "null models" << endl;
    ...
    if (occ.is_open() && voc.is_open())
    {
        ...
        for (uint8_t iStatistic = 0; iStatistic < maxStatistic; iStatistic++)
        {
            auto nullData = new uint64_t[fNRealizations * fNComponents];
            for (uint64_t i = 0; i < fNComponents * fNRealizations; i++)
                nullData[i] = 0;
            ...
            while (voc >> M)
            {
                for (uint64_t word = 0; word < M; word++)
                    counts[distr(rng)]++;
                printf("\r%lu/%lu", effectivelyLoadedRealization, fNRealizations);
